...
public class BookController {
    ...
    public Mono<SuccessResponseDTO> createBook(@RequestBody @Valid Mono<BookDTO> bookDTO, ServerWebExchange exchange) {
        return bookDTO
                .map(bookDTO1 -> new SuccessResponseDTO(bookDTO1, Translator.getMessage("success.save.entity", exchange)));
    }
    ...
    public Mono<SuccessResponseDTO> getOneBook(@PathVariable("id") Long id, ServerWebExchange exchange) {
        ...
        return Mono.just(new SuccessResponseDTO(new BookDTO(id,"Spring WebFlux", "1425IO4K8G", "Spring WebFlux Rest API internationalization i18n", 100, 200), Translator.getMessage("success.fetch.one", exchange)));
    }
